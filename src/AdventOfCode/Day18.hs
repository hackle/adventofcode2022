...
type Plane = (Coord, Coord, Coord, Coord)
...
-- > planesForCoord (0, 0, 0) offsets == offsets
-- True
-- > planesForCoord (1, 1, 1) (planesForCoord (-1, -1, -1) offsets) == offsets
-- True
planesForCoord :: [[Coord]] -> Coord -> [[Coord]]
planesForCoord bases c = fmap (fmap $ addCoords c) bases

overlapsForPlanes :: [[Coord]] -> M.Map [Coord] Int
overlapsForPlanes coordss = foldl (\aggr crds -> snd $ M.insertLookupWithKey updateCount crds 1 aggr) M.empty coordss
    where
